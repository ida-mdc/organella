...
times as many distances, so a batch of twenty is millions of rows - a few hundred lists
columnar, and more to assemble than to measure as dictionaries.
"""
...
from organella.model import DEEP_OBS_LEVEL, ROW_TYPE, ObjectMeasurement
...
# Declared rather than inferred, so a column no instance of one object could fill still
# arrives as the number it is - inferred it would be untyped nulls, which then mismatch
# another object's real values.
...
def deep_row_frames(
    ...
    Each frame is the measurer's columnar table plus the three columns a deep row needs to
    be found again: its object, its kind, and how deep it sits.
    """
    frames = []
...
def stacked(frames: Sequence[pl.DataFrame]) -> pl.DataFrame:
    ...
    diagonal_relaxed, so a column only one row kind fills comes through as nulls, and one
    typed in a frame and empty in another is widened rather than refused.
    """
    if not frames:
        return pl.DataFrame()
...
def _measurements_as_float32(table: pl.DataFrame) -> pl.DataFrame:
    ...
    Seven significant digits is more than a µm measurement carries, and this is ~12% of a
    real report's size.
    """
    casts = []
