...
Each target's distance transform is computed once per object and sampled at every
instance's voxels, which is why this is measured against the whole object rather than one
entity at a time.
"""
...
def segmented_center_um(
    ...
    With an ``--object-mask`` polarity is measured from that mask's centroid; without one,
    the centre of what was segmented is a better origin than the middle of whatever crop the
    images happen to have, and denying a run one lost it every polarity column.
...
def polarity_from_offset(offset: Sequence[float]) -> Dict[str, float]:
    ...
    if planar:
        ny, nx_ = units
        # The components as well as the angles, so a caller has a direction without
        # converting one.
