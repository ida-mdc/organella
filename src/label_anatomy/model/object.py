...
This is the only type the measuring code speaks, and it is deliberately a named thing
rather than a generic record with dimension inference and a metadata dict, which would only
hide what an object actually has. Everything an object knows is a named accessor here, and the fragment rule -
an object is measured whole - is checked once, on construction, rather than at the top of
every measurer.
"""
...
class ObjectStack:
    ...
    ``meta`` is what the loader found out about the object, in the shape the report's object
    row takes: the entity names and kinds, the files they came from, the voxel size, the
    object mask's centroid. It is passed through to the row rather than re-derived, which is
    why it stays a mapping instead of becoming twenty fields.
    """
    ...
    def __post_init__(self) -> None:
        ...
        An object's measurements are cross-entity and whole-region: a stack holding two of
        five entities, or a slab of the region, answers almost nothing, and answers it wrongly
        rather than partially. Checked here so the rule has one home - it used to be repeated,
        with three slightly different spellings, at the top of every measurer.
        """
...
class EntityVolume:
    ...
    A per-entity measurement sees only this. It still needs a few things the object as a
    whole holds - the voxel size, the object centre it is placed against - so it carries the
    stack rather than a copy of them.
    """
