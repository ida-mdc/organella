...
An object folder is TIFFs in this project's own data, but the same anatomy lives in the
formats the rest of the field publishes in: NIfTI for anything clinical, NRRD and MetaImage
for anything that went through ITK. Those are one ``SimpleITK.ReadImage`` away, and
SimpleITK is already a dependency, so the loader reads them rather than asking for a
conversion pass that would rewrite gigabytes to change nothing but the container.

Every reader here returns array order ``(Z, Y, X)`` for a volume and ``(Y, X)`` for a
plane, which is the order the rest of the package measures in. SimpleITK already hands
back that order; its *spacing* is the other way round, ``(x, y, z)``, and is reversed here
once so nothing downstream has to remember which convention it is holding.
"""
...
class ArrayRef:
    ...
    the point: a chunked array is read block by block, so a crop of one costs the blocks it
    overlaps and nothing else - a 512³ window of a 122-gigavoxel OpenOrganelle array is 8
    of its 1248 chunks, which is why a whole cell never has to be downloaded to measure a
...
def open_store(store: str):
    ...
    N5 and Zarr, local or over S3. Kept out of the base install: the analysis stack does
    not need them, so they are an extra and the message says so rather than a traceback
    ...
    if store.rstrip("/").endswith(".n5"):
        # zarr 3 dropped N5 without a replacement in zarr itself, which is why the extra
        # pins zarr<3. Its deprecation warning is not something a reader of a published N5
        # dataset can act on, so it does not go to their terminal.
        import warnings
...
# What SimpleITK reads and this package has a use for. Ordered longest-first so
# ``.nii.gz`` is recognised before ``.gz`` could be, and Analyze ``.hdr``/``.img`` pairs
# are left out: the header names the pixel file, so the pair is not one path.
ITK_SUFFIXES = (".nii.gz", ".nii", ".nrrd", ".nhdr", ".mha", ".mhd", ".mgz")

IMAGE_SUFFIXES = TIFF_SUFFIXES + ITK_SUFFIXES

# NIfTI stores spacing in millimetres whenever it says nothing else, and its unit field is
# very often 0 ("unknown") - it is 0 in TotalSegmentator. Assuming millimetres is what
# every reader of these files does; --voxel-size-um overrides it when that is wrong.
...
def _itk_dtype(pixel_id: int) -> np.dtype:
    ...
    Spelled out rather than taken from ``sitk.extra``, which is private: a header read is
    on the ``dry-run`` path for every file in a batch, and this is the only part of it
    that a SimpleITK release could quietly move.
    """
...
def read_header(path: Path | ArrayRef) -> Tuple[Tuple[int, ...], str]:
    ...
    ``dry-run`` promises headers only, and calls this for every file in a batch, so it
    stays a header read: SimpleITK's ``ReadImageInformation`` is ~6 ms on a 42-megavoxel
    NIfTI where reading the pixels is ~500 ms. For a store it is the array's metadata, so
    a remote object can be surveyed without fetching a chunk of it.
    """
    if isinstance(path, ArrayRef):
        array = _array(path)
        if path.box:
...
def _ref_voxel_size_um(ref: ArrayRef, ndim: int) -> Tuple[float, ...]:
    ...
    Two conventions are read, both used by OpenOrganelle: the ``multiscales`` transform on
    the parent group, which is per scale level and so the one that matters for a
    downsampled array, and ``pixelResolution`` on the array itself. Neither is invented -
    a store that carries neither gets an error naming what to pass instead.
    """
