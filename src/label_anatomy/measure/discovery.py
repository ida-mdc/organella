...
The naming rules that turn one object folder into one record. They came from
...
from label_anatomy.measure.readers import (
    ArrayRef,
    image_stem,
    is_image,
    read_voxel_size_um,
...
def split_kind_suffix(stem: str) -> tuple[str, str] | None:
    ...
    Only the trailing ``_label`` / ``_labels`` / ``_mask`` is read here. What remains is
    the *body*, and where the object's own prefix ends inside it is not decidable from
    the name alone - :func:`entity_name_from_body` needs the source file to say.
    """
...
def entity_name_from_body(body: str, source_norm: str, shared: int) -> str | None:
    ...
    ``shared`` is how much of the body the source file name accounts for. Splitting there
    rather than at the last underscore is what keeps a name that has underscores in it -
    ``s0011_rib_left_11`` is the entity ``rib_left_11``, not the entity ``11``, which
    would collide with ``rib_right_11`` and lose one of the two.
    """
...
def key_for_name(entities: Dict[str, Entity], name: str | None) -> str | None:
    ...
    Keys are ``<kind>:<name>``, but a kind can change after the key is made - a mask with
    several components is promoted to a label, and an ``auto`` entity only learns what it
    is when its pixels are read. The key is an identity, not a claim, so nothing rebuilds
    one from a kind it assumes.
    """
    if name is None:
        return None
...
# chunked store, the arrays inside it, and the window to read. That is what lets a 512³ crop
# of a 122-gigavoxel volume be measured at full resolution without the volume ever being
# downloaded - and the manifest is better provenance than the TIFFs would be, because it
# records exactly which store, scale and window a report came from.
...
def read_manifest(path: Path) -> Discovery:
    ...
    structure or many, so the kind is read off its content when the pixels are, exactly as
    for a file named by its structure alone.
    """
    from label_anatomy.measure.readers import ArrayRef, parse_box
...
# Subfolders that hold an object's entities named by nothing but the structure. This is
# how most published segmentations are laid out - TotalSegmentator ships
# ``<subject>/ct.nii.gz`` beside ``<subject>/segmentations/liver.nii.gz`` - and requiring
# the object's prefix on each of 117 files would mean rewriting gigabytes to rename them.
...
def inspect_object_dir(object_dir: Path, object_mask: str | None = None) -> Discovery:
    ...
    # Entities in a segmentations/ subfolder are named by the file alone - there is no
    # prefix to strip, and whether each is a label or a mask is read off its content when
    # the pixels are loaded rather than guessed from the name.
