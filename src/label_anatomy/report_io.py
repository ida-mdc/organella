...
It used to be PixelPatrol's format, written through its `save_parquet`, and the footer keys
carried its prefix. They are `anatomy_` now that the file is ours - but a report written
under the old prefix still reads, because :func:`read` accepts either. That costs one dict
lookup and saves anyone re-running a batch that took hours.

The descriptions are why the page can explain a metric under the chart of it:
`anatomy_column_descriptions` holds the whole column -> description map as JSON. The
per-field metadata carries it too, which is what makes the file self-describing to
`pyarrow` and friends, but a browser cannot reach that - DuckDB drops field metadata - so
the footer copy is the one the page reads.
"""
...
from label_anatomy.measure.loading import LOADER_NAME
...
#: Where the page reads the column descriptions from.
DESCRIPTIONS_KEY = "anatomy_column_descriptions"

#: What one measured thing is called, if the run said. Presentation only: the columns are
#: named `object_*` whatever this is, because renaming them per run would make two reports
#: of the same kind unjoinable.
NOUN_KEY = "anatomy_object_noun"

#: What the provenance keys are called. Reports written before the rename used ``pp_``,
#: and :func:`read` still answers to those.
FOOTER_PREFIX = "anatomy_"
LEGACY_FOOTER_PREFIX = "pp_"
...
def write(report: Report, output: Path, *, root: Path, paths: Sequence[str],
    ...
    logger.info("anatomy: wrote %s", output)
    return output
...
def read(report: Path) -> Tuple[Any, Dict[str, str]]:
    ...
    A report written before the rename has ``pp_`` keys, and they come back under the
    current names: the value is the same and a caller should not have to know which
    version wrote the file it was handed.
    """
    import polars as pl
    ...
    for key, value in raw.items():
        ...
        if name.startswith("ARROW:"):
            continue
        if name.startswith(LEGACY_FOOTER_PREFIX):
            name = FOOTER_PREFIX + name[len(LEGACY_FOOTER_PREFIX):]
            if name == f"{FOOTER_PREFIX}flavor":       # spelled the other way back then
                name = f"{FOOTER_PREFIX}flavour"
...
def recolour(report: Path, colours: Dict[str, str]) -> int:
    ...
    Colouring is a presentation choice and a run takes minutes, so a palette can be tried
    and changed against a report that already exists. `process --colours` writes it the same
    way, so there is one path either way, and the footer the run was written with is carried
    over untouched.
    """
    import polars as pl
    import pyarrow.parquet as pq
...
def _write(frame, dest: Path, footer: Dict[str, str], noun: Optional[str] = None) -> None:
    ...
    from label_anatomy.column_schema import descriptions_for
    ...
    for field in table.schema:
        ...
        fields.append(field)
    # The same map in the footer, because that is the copy a browser can read.
...
def _version() -> str:
    from label_anatomy import __version__
