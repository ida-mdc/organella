...
metadata, and the whole map goes in the footer as well, under
``anatomy_column_descriptions``. That second copy is what the report page reads - a browser
cannot get at field metadata, because DuckDB drops it and ``parquet_schema()`` does not
expose it - and it is why the explanation under a chart is the same sentence as the one in
the file rather than a second version of it kept somewhere else.

The declarations live next to the code that fills them, in :mod:`label_anatomy.measure`;
this module gathers them and adds the few columns the pipeline itself writes.
"""
...
from label_anatomy.measure.loading import LOADED_DESCRIPTIONS
from label_anatomy.measure.morphology import MorphologyMeasurer

# What a row is, and where it came from: stamped on by the pipeline rather than measured.
...
# The object's own file facts. Read off the folder while loading it, and described here
# because they are columns of the report like any other.
...
def describe(column: str) -> str:
    ...
    Nothing rather than a guess: a column with no description says so by having none, which
    is what lets a test find the ones a new measurer forgot.
    """
...
def with_noun(description: str, noun: Optional[str]) -> str:
    ...
    Done here, when the file is written, rather than in whatever reads it: the descriptions
    then say the same thing to the report page, to ``pyarrow``, and to anyone who opens the
    parquet in a notebook. Whole words only, so ``object_id`` in a sentence is left alone.
    """
    if not noun:
        return description
