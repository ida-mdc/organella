...
Nothing here raises. One object of a batch failing is ordinary - a folder missing the mask
it was told to use, a volume that will not read - and it must cost that object and nothing
else, so a failure comes back as :attr:`ObjectResult.error` for the batch to report by name.
"""
...
from label_anatomy.measure import (
    ContactMeasurer,
    GeometryWriter,
    InstanceMeasurer,
    MorphologyMeasurer,
...
from label_anatomy.model import ENTITY_ROW, OBJECT_ROW, ROW_TYPE, ObjectStack
from label_anatomy.pipeline import table

logger = logging.getLogger(__name__)

# Columns that describe the whole object, so they belong on its row and on no other: an
# entity row that carried the object's extent would read as if that were the entity's.
...
def _entity_rows(stack: ObjectStack, morphology: MorphologyMeasurer,
    ...
    Deliberately not keyed by the channel it was stacked as: a ``dim_c`` column would put a
    "slice by C" control in the viewer's sidebar, which reads an object's structures as if
    they were imaging channels - a five-structure cell offered as "5 channels".
    ``entity_name`` says which structure a row is about, which is what a widget groups by.
    """
...
def _extent(stack: ObjectStack) -> Dict[str, Any]:
    ...
    The spatial axes only: C is how many structures the object has, which ``n_entities``
    already says, and as a ``size_C`` column it reads as an image with five channels. The
    axis order is not a column either - it never varies, and ``spatial_dims`` says which
    axes an object has.
    """
...
def _provenance(folder: Path, stack: ObjectStack) -> Dict[str, Any]:
    ...
    An object is a folder, so the per-file columns a report usually carries (name, parent,
    depth, type) describe nothing here and are left out. ``file_extension`` is the format of
    the volumes themselves, which they share, so it stays: it tells a TIFF batch from any
    other.
    """
