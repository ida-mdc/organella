...
How wide the pool is comes from memory, not from cores (:func:`worker_count`): one of these
objects can need several gigabytes, and running more of them at once than fit is what
invites the OOM killer.

Which is not hypothetical, and is why this module is more than a ``pool.map``. SIGKILL gives
a worker no chance to report anything and breaks the pool for everyone; ``map`` then raises
and loses *every* result, including objects that finished hours earlier. So results are taken
as they complete (:func:`measure_in_pool`) and whatever the broken pool never ran is retried
narrower (:func:`measure_every_object`), because fewer large objects resident at once is
usually exactly the fix.
"""
...
def measure_every_object(work: Sequence[Work], excluded: Sequence[str], n_workers: int,
                         dispatch=None, on_result: OnResult = None) -> List[ObjectResult]:
    ...
    if _is_a_single_object(work, n_workers):
        return [_measured_here(work[0], excluded, on_result)]
...
def measure_in_pool(work: Sequence[Work], excluded: Sequence[str], n_workers: int,
                    on_result: OnResult = None) -> Tuple[List[ObjectResult], List[Work]]:
    """One round: what the pool measured, and what it never got to because it broke.

    Results are handled as they complete rather than collected at the end, which is what
    lets ``on_result`` put an object's rows on disk before anything else can go wrong. What
    the broken pool never ran comes back in submission order, so a retry meets the objects
    ...
    with ProcessPoolExecutor(max_workers=n_workers, mp_context=context,
        ...
        for future in as_completed(submitted):
            ...
            try:
                ...
                logger.error("anatomy: %s failed: %s: %s",
...
def _measured_here(one: Work, excluded: Sequence[str], on_result: OnResult) -> ObjectResult:
    folder, group = one
    result = measure_object(folder, group, excluded)
    if on_result is not None:
        on_result(result)
    return result
...
def _after_a_killed_worker(
    ...
    logger.error("anatomy: %s killed its worker (out of memory?); skipping it", folder.name)
